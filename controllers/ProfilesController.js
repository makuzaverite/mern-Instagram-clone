...
exports.createprofile = asyncHandler(async (req, res, next) => {
  ...
  if (!user)
    return next(new ErrorResponse(`No user found with id ${req.user._id}`, 400))
...
exports.unfollow = asyncHandler(async (req, res, next) => {
  ...
  const followIndex = existantance.followers.map((follower) =>
